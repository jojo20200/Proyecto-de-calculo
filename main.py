...
• Negativa: {area_data['porcentaje_negativo']:.1f}%"""
    ...
    def _calculate_area_detailed(self, func_str, a, b):
        ...
        # Área positiva (sobre el eje X)
        mask_pos = ys >= 0
        area_pos = sci_integrate.trapz(ys[mask_pos], xs[mask_pos])
       
        # Área negativa (bajo el eje X, en valor absoluto)
        mask_neg = ys < 0
        area_neg = abs(sci_integrate.trapz(ys[mask_neg], xs[mask_neg]))
