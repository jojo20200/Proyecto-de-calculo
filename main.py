...
class PhysicsEngine:
    ...
    def movimiento_proyectil(self, velocidad, angulo, altura_inicial=0,
        ...
        sol = solve_ivp(derivadas_proyectil, [0, 10],
                       ...
                       events=lambda t, y: y[1], rtol=1e-6, atol=1e-6) # Evento cuando y=0 (suelo)
...
def calcular_area_y_graficar(func_str, a=-5, b=5, puntos=1000):
    ...
    """
    print("\n" + "="*70)
    print("🧮 CÁLCULO DE ÁREA BAJO LA CURVA".center(70))
    print("="*70)
    ...
    print(f"1️⃣ Función ingresada: f(x) = {sp.pretty(expr)}")

    # Paso 2: Calcular integral simbólica
    try:
        integral_simb = sp.integrate(expr, (x, a, b))
        print(f"2️⃣ Integral simbólica: ∫ f(x) dx = {sp.pretty(integral_simb)}")
    except Exception:
        integral_simb = None
    ...
    try:
        ...
        print(f"3️⃣ Aproximación numérica: {area_num:.6f} ± {err:.2e}")
    except Exception as e:
    ...
    plt.fill_between(xs, ys, 0, where=(ys >= 0), color='red', alpha=0.35, label='Área positiva')
    plt.fill_between(xs, ys, 0, where=(ys < 0), color='red', alpha=0.15, label='Área negativa')
...
def _plot_function_con_area(self, func_str, xlim=(-5,5), color_index=0, animate=True, show_area=True, area_limits=None):
    """
    Extiende la función original de graficación para incluir
    el cálculo del área bajo la curva en un gráfico separado.
    """
    # Llamar al método original
    _original_plot_function(self, func_str, xlim, color_index, animate, show_area, area_limits)

    try:
        # Después de graficar, lanzar el análisis de área
        print("\n✨ Generando gráfico adicional de área bajo la curva...")
...
print("✅ [INTEGRACIÓN ACTIVA] Cada vez que se grafique una función, se calculará y mostrará el área bajo la curva.")
